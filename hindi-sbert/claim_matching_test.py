...
def main():
    ...
    for threshold in range(1, 101):
        laser_predictions = [1 if angdist(get_laser_embedding(pair['lookup_text']),
                                          get_laser_embedding(pair['database_text'])) > threshold / 100 else 0 for pair
                             in pairs]
        sbert_predictions = [1 if angdist(get_sbert_embedding(pair['lookup_text']),
                                          get_sbert_embedding(pair['database_text'])) > threshold / 100 else 0 for pair
                             in pairs]
